...
//try open initial pending orders
   tryOpenBuy();
   tryOpenSell();
// Re-open orders closed by take profit
   takeProfitBuy();
   takeProfitSell();
      ...
      if(Fractals_Down(3))
        {
         ...
         double TPS = NormalizeDouble(Low[3] - 15
         *Point,Digits);
      ...
      if(Fractals_Up(3))
        {
         ...
         double TPB=NormalizeDouble(High[3]+15*Point,Digits);                                  //order take-profit
...
bool takeProfitBuy()
  ...
  {
   int c;
   if(buyTicket!=0) // if we have open buy
     {
      if(OrderSelect(buyTicket,SELECT_BY_TICKET))
        {
         if(ORDER_TYPE==OP_BUY)
           {
            ...
            double SLB=NormalizeDouble(Low[c]-spread*Point,Digits);    //order stop-loss
            buyTicket=OrderSend(Symbol(),OP_BUY,LotTP,Ask,3,SLB,0);
            Alert("Order Buy");
...
bool takeProfitSell()
  ...
  {
   int c;
   if(sellTicket!=0) // if we have open sell
     {
      if(OrderSelect(sellTicket,SELECT_BY_TICKET))
        {
         if(ORDER_TYPE==OP_SELL)
           {
            if(OrderTakeProfit())          // when Take-Profit triggers - open partial sell order.  
            ...
            double SLS=NormalizeDouble(High[c]+spread*Point,Digits);    //order stop-loss
            buyTicket=OrderSend(Symbol(),OP_SELL,LotTP,Bid,3,SLS,0);
            Alert("Order Buy");

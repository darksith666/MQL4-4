...
#include <FR_UP_GREEN.mqh>                   //include fractals up
#include <FR_DOWN_GREEN.mqh>                 //include fractals down
      ...
      if(Fractals_Up_GR(3))
        {
         ...
         int c=4; //starting from candle 4 
         while(Fractals_Down_GR(c)==false)
           { //look for the nearest fractal down
            c++;
           }
         ...
         if(OrderType()==OP_SELL)
           {
            ...
            if(NormalizeDouble(OrderOpenPrice() -(150)*Point,Digits)==Ask && !Fractals_Up_GR(3)) //modify S/L to NULL
              {
               do
                 {
         ...
         if(OrderType()==OP_BUY)
           {
            ...
            if(NormalizeDouble(OrderOpenPrice()+150*Point,Digits)==Bid && !Fractals_Down_GR(3)) //modiy S/L to NULL
              {
               do
                 {
